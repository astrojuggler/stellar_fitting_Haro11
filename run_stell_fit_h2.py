...
fit2pop = Fit(pars, obs, syn, mask)
fit2pop.plot_obs(windows['ismLis'])

# generate the fitting object
    #MS changing nmax so that the mass (M=10**6 * n) does not exceed 10**7 Msun 
spops2 = [ StelPop ("Pop1", aval=6.477, afit=True, amin=6.0, amax=6.602, \
                     nfit=True, nmax=1000., \
                     zval=0.008, zfit=True, zmin=pars.metalLo, zmax=pars.metalHi), 
           StelPop ("Pop2", aval=7.4, afit=True, amin=6.602, amax=7.7, \
                     nfit=True, nmax=1000., \
                     zval=0.008, zfit=True, zmin=pars.metalLo, zmax=pars.metalHi) ]

# setup and pring the initial guesses
fit2pop.setup_npop_red(spops2, ebvval=0.1, ebvmin=pars.ebvLo, ebvmax=pars.ebvHi)  
fit2pop.lmpars.pretty_print()
fit2pop.guessnorm_npop_red(fit2pop.lmpars)
fit2pop.lmpars.pretty_print()

# do the fitting
fit2pop.fit_npop_red(minalg="differential_evolution")#, Nstep=pars.Nstep, Nwalker=pars.Nwalker, Nburn=pars.Nburn)  
fit2pop.show_fitres_npop_red()

# calculate the best spectra, write and plot the outputs
fit2pop.calc_bestspec_npop_red(fnOut=pars.fnRes2)
fit2pop.plot_npop_red(fnOut=pars.fnFigroot2)
fit2pop.write_energy_history(pars, fnOut=pars.fnSecHist2)
